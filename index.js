...
const ADMIN_ROLE = "1446194724074688724";
  ...
  try {
    ...
    if (i.commandName === "ticketpanel") {
      ...
      await i.channel.send({
        content: "To open a Ticket Click below 👇",
        components: [btn]
      });
    ...
    if (i.isButton() && i.customId === "ticket") {
      ...
      return i.reply({ content: "Select ticket type:", components: [menu], ephemeral: true });
    }
    ...
    if (i.commandName === "untimeout") {
      ...
      return i.reply({ content: "Timeout removed", ephemeral: true });
    }
    ...
    if (i.commandName === "unwarn") {
      ...
      if (!data || data.warns.length === 0)
        return i.reply({ content: "No warns", ephemeral: true });
    ...
    if (i.commandName === "clearwarn") {
      ...
      return i.reply({ content: "Warns cleared", ephemeral: true });
    }

    if (i.commandName === "warnlist") {
      const all = await Warn.find();
      const txt = all.map(x => `<@${x.userId}> - ${x.warns.length}`).join("\n");
      return i.reply({ content: txt || "No warns", ephemeral: true });
    }
